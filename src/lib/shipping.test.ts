...
describe('computeShipping — tiered rates', () => {
  it('is free at or above the free threshold ($800+)', () => {
  ...
  it('charges the mid rate ($50) from $500 up to (but not including) $800', () => {
    expect(computeShipping(MID_SHIPPING_THRESHOLD)).toBe(MID_SHIPPING_RATE) // exactly 500
    expect(computeShipping(650)).toBe(MID_SHIPPING_RATE)
    expect(computeShipping(FREE_SHIPPING_THRESHOLD - 0.01)).toBe(MID_SHIPPING_RATE) // 799.99
  })

  it('charges the base rate ($100) below $500', () => {
    expect(computeShipping(MID_SHIPPING_THRESHOLD - 0.01)).toBe(BASE_SHIPPING_RATE) // 499.99
